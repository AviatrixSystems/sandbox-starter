...
def launch_azure_state(command):
    ...
    data['processedData'].update(
        {'avaitrixTransitAzure': avaitrixTransitAzure})
...
def launch_transit(data, command):
    ...
    if command == 'yes':
        ...
        command_proc = ['bash', '-c', '. /root/sandbox_starter_web.sh;'
...
def aws_spoke_vpc(keyname):
    ...
    with subprocess.Popen(
        ...
        for line in proc_vpc.stdout:
            previous_lines.append(output_conversion_byline(line))
            progress = ''.join(previous_lines[-30:])
            status_json('spokeVPC', 'in-progress',
                        stored_state, None, progress)
            print(progress, end='')
...
def aws_azure_peering(command):
    ...
    if command == 'yes':
        command_bash = ['bash', '-c', '. /root/sandbox_starter_web.sh;'
